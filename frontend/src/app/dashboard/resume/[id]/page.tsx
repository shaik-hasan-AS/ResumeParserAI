...
export default function ResumeViewer() {
  ...
  useEffect(() => {
    const fetchFile = async () => {
      try {
        const res = await api.get(`/api/resume/${id}/file`, { responseType: 'blob' });
    ...
    <div className="min-h-screen bg-background relative p-4 md:p-8 pb-24 font-sans text-[16px] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start lg:h-[calc(100vh-140px)]">
          ...
          <div className="w-full h-[600px] lg:h-full rounded-3xl overflow-hidden border border-border shadow-xl bg-muted relative flex items-center justify-center">
            {fileUrl ? (
              <iframe 
                src={fileUrl}
