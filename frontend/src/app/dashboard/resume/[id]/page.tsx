...
const PDFViewer = dynamic(
  () => import('@react-pdf/renderer').then((mod) => mod.PDFViewer),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-full min-h-[600px] bg-muted/50 rounded-2xl border border-border animate-pulse flex items-center justify-center">
    ...
    <div className="min-h-screen bg-background relative p-4 md:p-8 pb-24 font-sans text-[16px] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-2xl shadow-sm border border-border">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              className={`hidden lg:flex h-9 px-4 rounded-lg font-semibold items-center gap-2 transition-all ${
                showPreview
        ...
        <div className={`grid grid-cols-1 gap-8 items-start transition-all duration-300 ${
          ...
          <div className={showPreview ? 'lg:col-span-3 space-y-4' : 'lg:col-span-4 space-y-4'}>
          ...
          <div className={showPreview ? 'lg:col-span-5 space-y-6' : 'lg:col-span-8 space-y-6'}>
            {!feedback ? (
          ...
          {/* Live Preview Column — only on lg+ when showPreview is true */}
          {showPreview && (
            <div className="hidden lg:block lg:col-span-4">
              <div className="sticky top-8 space-y-3">
                <div className="flex items-center justify-between px-1">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                    <Eye className="w-3.5 h-3.5 text-primary" /> Live PDF Preview
                  </p>
                  <span className="text-xs text-muted-foreground bg-muted border border-border px-2 py-0.5 rounded-full">
                    {pdfTheme}
                  </span>
                </div>
                <div className="rounded-2xl overflow-hidden border border-border shadow-2xl ring-1 ring-primary/10 bg-muted/30">
                  <PDFViewer
                    width="100%"
                    height={780}
                    showToolbar={false}
                  ...
                  >
                    <ResumePDF
                      parsedData={parsedData}
                      overrides={overrides}
                      aiRewrites={feedback?.structured_data?.bullet_point_rewrites}
                      structuredExperience={feedback?.structured_data?.structured_experience}
                      theme={pdfTheme}
                      aiSummary={feedback?.structured_data?.professional_summary}
                    />
                  </PDFViewer>
                </div>
                <p className="text-xs text-center text-muted-foreground/60 pt-1">
                  Updates live as you edit contact details ✦
                </p>
              </div>
            </div>
          )}
