...
export default function RecruiterDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background relative p-4 md:p-8 overflow-hidden font-sans">
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-indigo-500/10 rounded-full blur-[150px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-rose-500/10 rounded-full blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setShowCreateModal(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm rounded-xl h-10 px-6 font-semibold flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div className="bg-card border border-border rounded-3xl p-16 text-center flex flex-col items-center justify-center shadow-sm">
            ...
            <Button onClick={() => setShowCreateModal(true)} variant="outline" className="rounded-xl border-indigo-500/30 text-indigo-500 hover:bg-indigo-500/10">
              Post your first job
            </Button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {jobs.map((job) => (
              <div key={job.id} onClick={() => router.push(`/dashboard/recruiter/job/${job.id}`)} className="bg-card border border-border rounded-3xl p-6 shadow-sm hover:shadow-md hover:border-indigo-500/30 transition-all cursor-pointer group flex flex-col">
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-foreground mb-2 group-hover:text-indigo-500 transition-colors">{job.title}</h3>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border mt-auto">
                  ...
                  <div className="flex items-center gap-1 text-xs font-semibold text-indigo-500 bg-indigo-500/10 px-2 py-1 rounded-md border border-indigo-500/20">
        ...
        {showCreateModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            <div className="bg-card w-full max-w-2xl border border-border rounded-3xl p-8 shadow-2xl relative animate-in zoom-in-95 duration-200">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:ring-1 focus:ring-indigo-500 outline-none"
                  />
                </div>
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:ring-1 focus:ring-indigo-500 outline-none min-h-[250px] resize-y custom-scrollbar"
                  />
              ...
              <div className="flex justify-end gap-3 mt-8">
                <Button variant="ghost" onClick={() => setShowCreateModal(false)} className="rounded-xl">Cancel</Button>
                <Button onClick={handleCreateJob} disabled={creating || !newTitle || !newDescription} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-sm px-8">
